...
* Create an ssh key and copy it locally:
    ssh-keygen -R host
    ssh ubuntu@HOST "mkdir .ssh && ssh-keygen -f .ssh/id_rsa -N '' -q && cp .ssh/id_rsa.pub .ssh/authorized_keys"
    scp ubuntu@HOST:~/.ssh/id_rsa KEYFILE.key && chmod 600 KEYFILE.key
    ssh ubuntu@HOST "echo ubuntu | sudo -S sh -c 'echo ubuntu  ALL=\(ALL\) NOPASSWD:ALL >> /etc/sudoers'"
* Check it works (no password should be required)
    ssh -i KEYFILE.key ubuntu@HOST
...
== TODO
* Move nginx out into Machinesfile
* Fix nginx conf generation not working
* Make sure if desktop machine that ubuntu user is removed
* Set a userpass on the created user
...
* Remove old SSH keys when installing multiple times on the same IP
* Add timer
