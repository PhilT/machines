...
MACHINESFILE
  ...
  describe 'enter_password' do
    ...
    it 'prompts for a password' do
      should_receive(:ask).with('Enter type password: ').once.and_return 'pass'
      enter_password('type')
    end

    it 'returns password' do
      enter_password('type').should == 'pass'
    end

    it 'adds to password list' do
      enter_password('type')
      AppConf.passwords.should == ['pass']
    end

    it 'repeats until password and confirmation match' do
      should_receive(:ask).with('Enter type password: ').twice.and_return 'pass'
    ...
    it 'password still returned when not confirming' do
      enter_password('type', false).should == 'pass'
    end
